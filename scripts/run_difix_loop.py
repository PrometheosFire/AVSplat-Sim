...
def resolve_schedule(loop_cfg: DictConfig) -> List[List[List[float]]]:
    ...
    The generated ramp accumulates magnitudes, so round r renders every level up
    to r+1 rather than only the newest. Each round's pseudo-GT is therefore
    regenerated from the freshest model instead of carrying stale pixels made by
    an earlier, weaker one.
    """
    ...
    for r in range(n_rounds):
        schedule.append([[step[i] * (k + 1) for i in range(3)] for k in range(r + 1)])
    return schedule
...
def run_render(
    ...
    """Render the control trajectory plus this round's shifted ones."""
...
def run_difix(
    ...
    """Clean this round's slice and emit its manifest."""
    ...
    print(f"  [difix] stride={stride} phase={phase}")
    _run(
        [
            GSPLAT_PYTHON,
...
def main(cfg: DictConfig) -> None:
    ...
    # --- The loop ---
    manifests: List[str] = []
    state: Dict[str, Any] = {"rounds": [], "loop_dir": loop_dir, "cameras": cameras}
    ...
    for r in range(n_rounds):
        ...
        if not is_final:
            render_dir = run_render(
                round_dir, train_dir, schedule[r], cameras, overrides, env
            )
            manifest = run_difix(
                round_dir, render_dir, real_bank_dir, cameras, r, stride,
                phases[r], overrides, env,
            )
            ...
            record.update(
                {"render_dir": render_dir, "manifest": manifest,
                 "shifts": schedule[r], "phase": phases[r]}
            )
    ...
    print(f"\n{'=' * 60}\nLoop complete. Final model: {state['rounds'][-1]['train_dir']}")
